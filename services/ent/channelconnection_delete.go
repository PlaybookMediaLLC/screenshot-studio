...
// ChannelConnectionDelete is the builder for deleting a ChannelConnection entity.
type ChannelConnectionDelete struct {
	config
	hooks    []Hook
...
func (_d *ChannelConnectionDelete) sqlExec(ctx context.Context) (int, error) {
	_spec := sqlgraph.NewDeleteSpec(channelconnection.Table, sqlgraph.NewFieldSpec(channelconnection.FieldID, field.TypeString))
...
func (_d *ChannelConnectionDeleteOne) Exec(ctx context.Context) error {
	...
	case err != nil:
		return err
	case n == 0:
		return &NotFoundError{channelconnection.Label}
	default:
		return nil
	}
}
