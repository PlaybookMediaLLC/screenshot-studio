...
function getSafeSegment(value: string): string {
  if (
    ...
    value === "." ||
    value === ".." ||
    INVALID_PATH_SEGMENT.test(value)
  ) {
...
export async function createTenantUploadUrl(
  input: TenantUploadRequest
): Promise<TenantUploadUrl> {
  ...
  const response = await fetch(
    getRequestUrl(`/object/upload/sign/${bucket}/${objectKey}`),
    {
