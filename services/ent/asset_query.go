...
type AssetQuery struct {
	...
	withVariants *CreativeVariantQuery
...
func (_q *AssetQuery) QueryVariants() *CreativeVariantQuery {
	query := (&CreativeVariantClient{config: _q.config}).Query()
...
func (_q *AssetQuery) WithVariants(opts ...func(*CreativeVariantQuery)) *AssetQuery {
	query := (&CreativeVariantClient{config: _q.config}).Query()
...
//
//	client.Asset.Query().
//		GroupBy(asset.FieldOrganizationID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Asset.Query().
//		Select(asset.FieldOrganizationID).
//		Scan(ctx, &v)
...
func (_q *AssetQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Asset, error) {
	...
	if query := _q.withVariants; query != nil {
		if err := _q.loadVariants(ctx, query, nodes,
			func(n *Asset) { n.Edges.Variants = []*CreativeVariant{} },
			func(n *Asset, e *CreativeVariant) { n.Edges.Variants = append(n.Edges.Variants, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AssetQuery) loadVariants(ctx context.Context, query *CreativeVariantQuery, nodes []*Asset, init func(*Asset), assign func(*Asset, *CreativeVariant)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(creativevariant.FieldSourceAssetID)
	}
	query.Where(predicate.CreativeVariant(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(asset.VariantsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "source_asset_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
