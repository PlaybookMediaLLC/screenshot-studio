...
function extendMaterial(
  ...
  const baseDefines =
    ("defines" in physical
      ? (physical as { defines?: Record<string, string> }).defines
      : undefined) ?? {};
...
function hexToNormalizedRGB(hex: string): readonly [number, number, number] {
...
const MergedPlanes = forwardRef<THREE.Mesh, MergedPlanesProps>(
  ({ material, width, count, height }, ref): React.JSX.Element => {
...
const PlaneNoise = forwardRef<THREE.Mesh, MergedPlanesProps>(
